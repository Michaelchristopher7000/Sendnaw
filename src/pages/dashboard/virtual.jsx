...
const SENDNAW_LOGO = "/images/SendNaw_logo_main-removebg-preview.png";
...
function CardSVG({ design, holderName, cardNumber, expiry, masked = true, flipped = false, small = false }) {
  ...
  const W = small ? 280 : "100%";
  const H = small ? 176 : "auto";
  const displayNumber = masked ? maskCardNumber(cardNumber) : formatCardDisplay(cardNumber);

  const cardStyle = small
    ? { width: W, height: H, perspective: 1000, cursor: "pointer", flexShrink: 0 }
    : { width: "100%", maxWidth: 420, perspective: 1000, cursor: "pointer" };

  return (
    <div style={cardStyle}>
      <div style={{
        width: "100%",
        paddingBottom: small ? 0 : "63%",
        height: small ? H : 0,
        position: "relative",
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", top: -40, right: -40, width: small ? 140 : 200, height: small ? 140 : 200, borderRadius: "50%", background: d.circles[0] }} />
          <div style={{ position: "absolute", bottom: -50, left: -50, width: small ? 160 : 220, height: small ? 160 : 220, borderRadius: "50%", background: d.circles[1] }} />

          <div style={{ position: "relative", zIndex: 1, padding: small ? "16px 18px" : "24px 28px", height: "100%", boxSizing: "border-box", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <div style={{ width: small ? 36 : 48, height: small ? 28 : 36, borderRadius: 5, background: "linear-gradient(145deg, #E8C84A, #F5D96B, #C8A030)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "inset 0 1px 2px rgba(255,255,255,0.5)", position: "relative", overflow: "hidden" }}>
            ...
            <div>
              <div style={{ fontSize: small ? 13 : 19, fontWeight: 600, letterSpacing: small ? "0.12rem" : "0.2rem", fontFamily: "monospace", color: d.textColor, textShadow: "0 1px 2px rgba(0,0,0,0.2)" }}>
                {displayNumber}
              </div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              <div style={{ display: "flex", gap: small ? 14 : 24 }}>
                <div>
                  <div style={{ fontSize: small ? 7 : 9, color: "rgba(255,255,255,0.65)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 2 }}>Card Holder</div>
                  <div style={{ fontSize: small ? 9 : 13, fontWeight: 700, color: d.textColor, letterSpacing: "0.5px" }}>{holderName?.toUpperCase() || "YOUR NAME"}</div>
                </div>
                <div>
                  ...
                  <div style={{ fontSize: small ? 9 : 13, fontWeight: 700, color: d.textColor }}>{expiry}</div>
                </div>
              </div>
              {d.network === "VISA" ? (
        ...
        {/* Back — only render for full-size flippable cards */}
        {!small && (
          <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)", borderRadius: 20, background: d.gradient, overflow: "hidden", boxShadow: "0 25px 50px rgba(0,0,0,0.35)" }}>
            <div style={{ position: "absolute", top: 40, left: 0, right: 0, height: 48, background: "#111" }} />
            <div style={{ position: "absolute", top: 108, left: 28, right: 28 }}>
              <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 6, padding: "8px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)" }}>CVV</div>
                <div style={{ fontFamily: "monospace", fontSize: 18, fontWeight: 700, color: "#fff", letterSpacing: "0.3rem" }}>•••</div>
              </div>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 8, textAlign: "center" }}>This card is property of SendNaw Technologies</div>
            </div>
            <div style={{ position: "absolute", bottom: 22, right: 28, display: "flex", alignItems: "center", gap: 6 }}>
              <img src={SENDNAW_LOGO} alt="SendNaw" style={{ width: 22, height: 22, borderRadius: "50%", objectFit: "contain" }} />
              <span style={{ color: "#fff", fontWeight: 800, fontSize: 14 }}>SendNaw</span>
            </div>
          </div>
        )}
      </div>
...
function PageHeader({ title, subtitle, onBack, colors }) {
  return (
...
// ─── Step indicator ──────────────────────────────────────────────────────────
function StepBar({ current, total, colors }) {
  return (
    <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{
          flex: 1, height: 4, borderRadius: 99,
          background: i < current ? "linear-gradient(90deg, #7B2FBE, #C060E8)" : colors.border,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {CARD_DESIGNS.map((d) => (
          <div
            ...
            style={{
              cursor: "pointer",
              border: selected === d.id ? "2.5px solid #7B2FBE" : `1.5px solid ${colors.border}`,
              borderRadius: 16,
              padding: 12,
              display: "flex",
              alignItems: "center",
              gap: 14,
              background: selected === d.id ? "#7B2FBE08" : colors.cardBg,
              transition: "all 0.2s",
            }}
          >
            {/* Mini card preview strip */}
            <div style={{ width: 56, height: 36, borderRadius: 8, background: d.gradient, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ color: "#fff", fontSize: 8, fontWeight: 800, letterSpacing: 0.5 }}>{d.network}</span>
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: colors.text }}>{d.name}</p>
              <p style={{ margin: 0, fontSize: 12, color: colors.textSecondary }}>{d.network} · Virtual Debit</p>
            </div>
            {selected === d.id && <i className="bi bi-check-circle-fill" style={{ color: "#7B2FBE", fontSize: 18 }} />}
          </div>
        ))}
      </div>
    </div>
...
// ─── Create Virtual Card — Full Page ────────────────────────────────────────
function CreateVirtualPage({ onBack, onCreated, userFullName, colors }) {
  ...
  return (
    <div style={{ fontFamily: font }}>
      <PageHeader
        ...
        colors={colors}
      />
      <StepBar current={step} total={2} colors={colors} />
      ...
      {step === 1 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          {/* Currency picker */}
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, color: colors.textSecondary, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>Card currency</p>
            <div style={{ position: "relative" }}>
              {selectedCurrency && (
                <img
                  src={selectedCurrency.flag}
                  alt={selectedCurrency.code}
                  style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", width: 28, height: 20, objectFit: "cover", borderRadius: 3, zIndex: 1 }}
                />
              )}
              <select
                value={currency}
          ...
          <button
            onClick={() => setStep(2)}
            style={{ width: "100%", padding: "15px", borderRadius: 40, background: "linear-gradient(135deg, #7B2FBE, #C060E8)", border: "none", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: font }}
          >
      ...
      {step === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {/* Card preview */}
          <div style={{ background: colors.lightGray, borderRadius: 20, padding: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
            <p style={{ fontSize: 12, color: colors.textSecondary, margin: 0, fontWeight: 600 }}>Card preview</p>
            <div style={{ width: "100%", maxWidth: 340 }}>
              <CardSVG design={design} holderName={userFullName} cardNumber="" expiry="MM/YY" masked small />
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
              <span style={{ fontSize: 13, background: "#7B2FBE18", color: "#7B2FBE", borderRadius: 40, padding: "4px 12px", fontWeight: 600 }}>
                {CARD_DESIGNS.find(d => d.id === design)?.name}
              </span>
              <span style={{ fontSize: 13, background: colors.cardBg, color: colors.textSecondary, borderRadius: 40, padding: "4px 12px", fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
                <img src={selectedCurrency?.flag} alt={selectedCurrency?.code} style={{ width: 20, height: 14, objectFit: "cover", borderRadius: 2 }} />
          ...
          {/* Warning */}
          <div style={{ background: `${colors.warning}18`, borderRadius: 14, padding: "14px 16px", display: "flex", gap: 10 }}>
            <i className="bi bi-exclamation-triangle" style={{ color: colors.warning, flexShrink: 0, marginTop: 1 }} />
            <span style={{ fontSize: 13, color: colors.warning, lineHeight: 1.6 }}>
              Save your card details once issued — the full number and CVV won't be shown again.
            </span>
          </div>

          {/* Summary list */}
          <div style={{ background: colors.cardBg, borderRadius: 14, border: `1px solid ${colors.border}`, overflow: "hidden" }}>
            {[
              { label: "Design", value: CARD_DESIGNS.find(d => d.id === design)?.name },
              { label: "Currency", value: `${selectedCurrency?.code} – ${selectedCurrency?.name}` },
              { label: "Card type", value: "Virtual Debit" },
              { label: "Network", value: CARD_DESIGNS.find(d => d.id === design)?.network },
          ...
          <button
            ...
            disabled={loading}
            style={{ width: "100%", padding: "15px", borderRadius: 40, background: "linear-gradient(135deg, #7B2FBE, #C060E8)", border: "none", color: "#fff", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", fontFamily: font, opacity: loading ? 0.8 : 1 }}
          >
            {loading ? <><i className="bi bi-hourglass-split" /> Issuing card...</> : <><i className="bi bi-credit-card" /> Issue card instantly</>}
          </button>
        </div>
      )}
    </div>
...
// ─── Physical Card Request — Full Page ──────────────────────────────────────
function PhysicalCardPage({ onBack, onSubmit, userFullName, colors }) {
  const [step, setStep] = useState(1);
  const [design, setDesign] = useState("primary");
  ...
  const stepLabels = ["Choose design", "Personal details", "Delivery address"];

  const handleSubmit = async () => {
    const required = ["fullName", "phone", "address", "city", "state", "idNumber", "dob"];
    for (const k of required) {
      if (!form[k].trim()) {
        showNotification("SendNaw", `Please fill in ${k.replace(/([A-Z])/g, ' $1').toLowerCase()}`, "error");
        return;
      }
    }
    setLoading(true);
  ...
  return (
    <div style={{ fontFamily: font }}>
      <PageHeader
        ...
        colors={colors}
      />
      <StepBar current={step} total={3} colors={colors} />

      {step === 1 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <DesignPicker selected={design} onChange={setDesign} holderName={userFullName} colors={colors} />

          <div style={{ background: `${colors.info}15`, borderRadius: 14, padding: "14px 16px", display: "flex", gap: 10 }}>
            <i className="bi bi-info-circle" style={{ color: colors.info, flexShrink: 0, marginTop: 2 }} />
            <span style={{ fontSize: 13, color: colors.info, lineHeight: 1.6 }}>
              Physical cards are delivered within 7–14 business days. A delivery fee applies.
            </span>
          </div>
      ...
      {step === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>Full legal name</label>
            <input style={inputStyle} value={form.fullName} onChange={e => up("fullName", e.target.value)} placeholder="As it appears on your ID" />
          </div>
          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>Phone number</label>
            <input style={inputStyle} value={form.phone} onChange={e => up("phone", e.target.value)} placeholder="+234 000 000 0000" />
          </div>
          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>Date of birth</label>
            <input style={inputStyle} type="date" value={form.dob} onChange={e => up("dob", e.target.value)} />
          </div>
          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>Government ID type</label>
          ...
          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>{form.idType} number</label>
            <input style={inputStyle} value={form.idNumber} onChange={e => up("idNumber", e.target.value)} placeholder="Enter your ID number" />
          </div>

          <button
            onClick={() => {
              if (!form.fullName || !form.phone || !form.idNumber || !form.dob) {
                showNotification("SendNaw", "Please fill all fields", "error");
                return;
              }
              setStep(3);
            }}
            style={{ width: "100%", padding: "15px", borderRadius: 40, background: "linear-gradient(135deg, #7B2FBE, #C060E8)", border: "none", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: font, marginTop: 4 }}
          >
            Continue <i className="bi bi-arrow-right" />
          </button>
        </div>
      ...
      {step === 3 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>Street address</label>
            <input style={inputStyle} value={form.address} onChange={e => up("address", e.target.value)} placeholder="House number, street name" />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>City</label>
              <input style={inputStyle} value={form.city} onChange={e => up("city", e.target.value)} placeholder="Lagos" />
            </div>
            <div>
              <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>State</label>
              <input style={inputStyle} value={form.state} onChange={e => up("state", e.target.value)} placeholder="Lagos State" />
            </div>
          </div>

          <div>
            <label style={{ fontSize: 13, color: colors.textSecondary, display: "block", marginBottom: 6, fontWeight: 500 }}>Country</label>
          ...
          {/* Delivery summary */}
          <div style={{ background: colors.lightGray, borderRadius: 16, padding: 16 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: colors.text, margin: "0 0 10px" }}>Delivery summary</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { icon: "📦", label: "Delivery time", value: "7–14 business days" },
                { icon: "💳", label: "Card design", value: CARD_DESIGNS.find(d => d.id === design)?.name },
                { icon: "📍", label: "Shipping to", value: `${form.city || "—"}, ${form.state || "—"}, ${form.country}` },
          ...
          <button
            ...
            disabled={loading}
            style={{ width: "100%", padding: "15px", borderRadius: 40, background: "linear-gradient(135deg, #7B2FBE, #C060E8)", border: "none", color: "#fff", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", fontFamily: font, opacity: loading ? 0.8 : 1, marginTop: 4 }}
          >
            {loading ? <><i className="bi bi-hourglass-split" /> Submitting...</> : <><i className="bi bi-send" /> Submit request</>}
          </button>
        </div>
      )}
    </div>
...
function EmptyCardState({ onCreateVirtual, onRequestPhysical, colors }) {
  return (
    <div style={{ textAlign: "center", padding: "1rem 0" }}>
      <div style={{ position: "relative", width: 240, margin: "0 auto 2rem", height: 160 }}>
        <div style={{ position: "absolute", top: 24, left: 24, width: 200, height: 130, borderRadius: 18, background: "linear-gradient(145deg, #7B2FBE22, #C060E822)", border: `1.5px dashed ${colors.border}`, transform: "rotate(-6deg)" }} />
        <div style={{ position: "absolute", top: 12, left: 12, width: 200, height: 130, borderRadius: 18, background: "linear-gradient(145deg, #7B2FBE33, #C060E833)", border: `1.5px dashed ${colors.border}`, transform: "rotate(-3deg)" }} />
        <div style={{ position: "absolute", top: 0, left: 0, width: 200, height: 130, borderRadius: 18, background: colors.cardBg, border: `1.5px dashed ${colors.border}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: 360, margin: "0 auto" }}>
        <button onClick={onCreateVirtual} style={{ background: "linear-gradient(135deg, #7B2FBE, #C060E8)", border: "none", borderRadius: 40, padding: "15px 24px", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 8px 20px rgba(123,47,190,0.3)", fontFamily: font }}>
          <i className="bi bi-phone" /> Create virtual card — instant
        </button>
        <button onClick={onRequestPhysical} style={{ background: colors.cardBg, border: `1.5px solid ${colors.border}`, borderRadius: 40, padding: "15px 24px", color: colors.text, fontWeight: 600, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: font }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 24, flexWrap: "wrap" }}>
        {[["bi-shield-check", "Secure & encrypted"], ["bi-globe", "Works worldwide"], ["bi-lightning", "Instant issuance"]].map(([icon, label]) => (
          <div key={icon} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: colors.textSecondary }}>
            <i className={`bi ${icon}`} style={{ color: "#7B2FBE" }} />{label}
          </div>
        ))}
      </div>
    </div>
...
// ─── New card issued modal (keep as overlay — it's small and info-only) ──────
function NewCardModal({ cardData, onClose, colors }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ background: colors.cardBg, borderRadius: 28, padding: "1.8rem", width: "100%", maxWidth: 400, fontFamily: font, textAlign: "center" }}>
        ...
        {[
          { label: "Card number", value: cardData.card_number, field: "number" },
          { label: "CVV", value: cardData.cvv, field: "cvv" },
          { label: "Expiry", value: cardData.expiry, field: "expiry" },
...
export default function VirtualCard() {
  ...
  // view: "main" | "create-virtual" | "request-physical"
  ...
  // ── Sub-pages ──────────────────────────────────────────────────────────────
  if (view === "create-virtual") {
    return (
      <div style={{ minHeight: "100vh", background: colors.darkBg, fontFamily: font, padding: "1.5rem 1rem" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <CreateVirtualPage
            onBack={() => setView("main")}
            onCreated={(data) => { setNewCardData(data); setView("main"); fetchCard(); }}
            userFullName={userFullName}
            colors={colors}
          />
        </div>
        {newCardData && <NewCardModal cardData={newCardData} onClose={() => setNewCardData(null)} colors={colors} />}
      </div>
    );
  }

  if (view === "request-physical") {
    return (
      <div style={{ minHeight: "100vh", background: colors.darkBg, fontFamily: font, padding: "1.5rem 1rem" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <PhysicalCardPage
            onBack={() => setView("main")}
            onSubmit={() => { setView("main"); setPhysicalSuccess(true); }}
            userFullName={userFullName}
            colors={colors}
          />
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: colors.darkBg, fontFamily: font, padding: "1.5rem 1rem" }}>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {!loading && !card && (
          <EmptyCardState
            onCreateVirtual={() => setView("create-virtual")}
            onRequestPhysical={() => setView("request-physical")}
            colors={colors}
          />
        )}
        ...
        {!loading && card && (
          <>
            {/* Card display — full width, responsive */}
            ...
            <div style={{ background: colors.cardBg, borderRadius: 20, padding: "1.25rem", border: `1px solid ${colors.border}`, marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
                <div>
                  ...
                  <p style={{ fontSize: 11, color: colors.textSecondary, margin: 0, display: "flex", alignItems: "center", gap: 4 }}>
                    <img src={curr.flag} alt={curr.code} style={{ width: 20, height: 14, objectFit: "cover", borderRadius: 2 }} /> {curr.code} · Virtual
                  </p>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {[
                  ...
                  <button key={label} onClick={action} style={{ padding: "10px 4px", borderRadius: 14, border: "none", background: `${bg}18`, color: bg, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, fontFamily: font }}>
            ...
            {/* Physical card CTA */}
            <div
              onClick={() => setView("request-physical")}
              style={{ background: "linear-gradient(135deg, #7B2FBE18, #C060E818)", borderRadius: 16, padding: "1rem 1.25rem", border: "1px solid #7B2FBE33", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}
            >
      ...
      {showFund && card && <FundModal onClose={() => setShowFund(false)} onFund={fundCard} loading={funding} currency={card.currency || "USD"} colors={colors} />}
      {showCvv && card && <CvvModal cvv={card.cvv} onClose={() => setShowCvv(false)} colors={colors} />}
      {newCardData && <NewCardModal cardData={newCardData} onClose={() => setNewCardData(null)} colors={colors} />}
    </div>
  );
}